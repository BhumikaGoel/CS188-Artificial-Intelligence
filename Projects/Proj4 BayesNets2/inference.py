...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        #The algorithm should iterate over hidden variables in elimination order,
        # performing joining over and eliminating that variable, until the only 
        #the query and evidence variables remain.

        #The sum of the probabilities in your output factor should sum to one 
        #(so that it is a true conditional probability, conditioned on the evidence).

        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for var in eliminationOrder:
            factors, result = joinFactorsByVariable(factors, var)
            unconditional_vars_lst = result.unconditionedVariables()
            if len(unconditional_vars_lst) > 1:
                eliminateFactor = eliminate(result, var)
                factors.append(eliminateFactor)
        join_factors = joinFactors(factors)

        return normalize(join_factors)
