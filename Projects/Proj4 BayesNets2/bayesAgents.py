...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    def valueAtPosition(position, gameState):
        # 8      |
        # 7  T_L | T_R
        # 6      |
        # 5______|_______
        ...
        X_Val = gameState.data.layout.width/2.0
        Y_Val = gameState.data.layout.height/2.0
        if position[0] < X_Val and position[1] > Y_Val:
            return TOP_LEFT_VAL
        if position[0] < X_Val and position[1] < Y_Val:
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                ...
                BERKELEY = 0
                SJSU = 0
                if assignment[FOOD_HOUSE_VAR] == valueAtPosition(obsPos, gameState):
                    STANFORD = PROB_FOOD_RED
                    BERKELEY = 1 - PROB_FOOD_RED
                    #obsFactor.setProbability(assignment, BERKELEY)
                elif assignment[GHOST_HOUSE_VAR] == valueAtPosition(obsPos, gameState):
                    STANFORD = PROB_GHOST_RED
                    BERKELEY = 1 - PROB_GHOST_RED
                    #obsFactor.setProbability(assignment, STANFORD)
                else:
                    SJSU = 1
                    #obsFactor.setProbability(assignment, SJSU)

                obsColor = assignment[obsVar]
                if obsColor == RED_OBS_VAL:
                    obsFactor.setProbability(assignment, STANFORD)
                elif obsColor == BLUE_OBS_VAL:
                    obsFactor.setProbability(assignment, BERKELEY)
                else:
                    obsFactor.setProbability(assignment, SJSU)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignment in result.getAllPossibleAssignmentDicts():
        result_prob = result.getProbability(assignment)
        if prob < result_prob:
            prob = result_prob
            foodHousePos = assignment
    return foodHousePos
...
class VPIAgent(BayesAgent):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for assignment in factor.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                ...
                leftExpectedValue = (prob_left * WON_GAME_REWARD) + ((1 - prob_left) * GHOST_COLLISION_REWARD)
                ...
                rightExpectedValue = (prob_right * WON_GAME_REWARD) + ((1 - prob_right) * GHOST_COLLISION_REWARD)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        #Your solution to computeExploreValue can rely on your 
        #solution to computeEnterValues to determine the value of future observations.
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            maxVal = max(self.computeEnterValues(explorationEvidence, enterEliminationOrder))
