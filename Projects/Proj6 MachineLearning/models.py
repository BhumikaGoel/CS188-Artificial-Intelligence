...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        self.learning_rate = 0.02 #between 0.001 and 1.0
...
class LanguageIDModel(Model):
    ...
    def __init__(self):
        ...
        # You may use any learning rate that works well for your architecture
        "*** YOUR CODE HERE ***"
        self.learning_rate = 0.03 #0.03 #between 0.001 and 1.0
        self.hidden_layer_size = 90 #100 #between 10 and 400
        #to implement f(x) = relu(x.w1+b1).w2 + b2
        self.w1 = nn.Variable(self.num_chars, self.hidden_layer_size)
        self.b1 = nn.Variable(self.hidden_layer_size)
        l = len(self.languages)
        self.w2 = nn.Variable(self.hidden_layer_size, l)
        self.b2 = nn.Variable(l)
    ...
    def run(self, xs, y=None):
        ...
        #An RNN begins by initializing a d-sized vector h0, for example
        #to an all-zero vector (though you can make the initial value a trainable parameter).
        #In batched form, the vector  is replaced with the matrix  of dimensionality .
        #Note that every row of this matrix must be the same.
        #If you are having difficulty constructing , start by making it be an
        #all-zero matrix of correct shape.
        h_0 = np.zeros((batch_size, self.hidden_layer_size))

        #It then applies a function that takes as input  and a vector
        #representation of the first character in the word, and outputs
        #a new state vector .
        input_h_0 = nn.Input(graph, h_0)


        #This process then repeats for all characters in the input word:
        for xs_char in xs:

            input_x = nn.Input(graph, xs_char)
            #input_y = Input(graph, y)
            #a = x.w1
            a = nn.MatrixMultiply(graph, input_x, self.w1)
            #relu(a+b1).w2 + b2
            #b = a + b1
            b = nn.MatrixVectorAdd(graph, a, self.b1)
            #Be sure to combine information from  and  prior to applying any nonlinearity
            #b_h = b + input_h_0
            b_h = nn.Add(graph, b, input_h_0)
            #relu(b_h).w2 + b2
            two_layer_relu = nn.ReLU(graph, b_h)
            #h1 = f(h0, c0)
            input_h_0 = two_layer_relu

        #c = relu(b_h).w2
        c = nn.MatrixMultiply(graph, input_h_0, self.w2)
        #d = c + b2
        d = nn.MatrixVectorAdd(graph, c, self.b2)
