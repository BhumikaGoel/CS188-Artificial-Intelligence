...
class Graph(object):
    ...
    def __init__(self, variables):
        ...
        for variab in variables:
            self.add(variab)
        #self.node_derivatives[variables[-1]] = 1.0
    ...
    def get_inputs(self, node):
        ...
        for parent in parents:
            if type(parent) is Variable:
                lst.append(parent.data)
            else:
                lst.append(self.node_outputs[parent])
        return lst
    ...
    def get_gradient(self, node):
        ...
        "*** YOUR CODE HERE ***"
        if self.backbool:
            #print("backbool is called")
            return self.node_derivatives[node]
        else:
            return self.node_derivatives[node]
    ...
    def add(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.lstofvariables.append(node)
        self.node_parents[node] = node.get_parents()

        # parents = self.node_parents[node]
        # parent_data = [par.data for par in parents]
        # self.node_outputs[node] = node.forward(parent_data)

        self.node_outputs[node] = node.forward(self.get_inputs(node))

        self.node_derivatives[node] = np.zeros_like(self.node_outputs[node])
    ...
    def backprop(self):
        ...
        self.backbool = True
        for i in range(1, len(self.lstofvariables) + 1):
            node = self.lstofvariables[-i]
            if i == 1:
                self.node_derivatives[node] = 1.0
                back_derivates = node.backward(self.get_inputs(node), 1.0)

            else:
                #parents = self.node_parents[node]
                node_derivative = self.node_derivatives[node]
                back_derivates = node.backward(self.get_inputs(node), node_derivative)
                #print (back_derivates)
                # for k in range(len(parents)):
                #     self.node_derivatives[parents[k]] = back_derivates[k]
                #     #print (self.node_derivatives[parents[k]])
            parents = self.node_parents[node]
            for k in range(len(parents)):
                self.node_derivatives[parents[k]] += back_derivates[k]
    ...
    def step(self, step_size):
        ...
        """
        "*** YOUR CODE HERE ***"
        for variab in self.lstofvariables:
            if type(variab) is Variable:
                variab.data = variab.data - self.node_derivatives[variab] * step_size
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        matrix_A = inputs[0]
        matrix_B = inputs[1]
        return [np.dot(gradient, matrix_B.T), np.dot(matrix_A.T, gradient)]
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        # # agrad = np.reshape(gradient, a.shape)
        # # xgrad = np.reshape(gradient, x.shape)
        # return [gradient, gradient*np.ones_like(x)]
        # print (inputs)
        # print (gradient)
        #return [agrad, xgrad]
        if gradient is np.ones(gradient.shape):
            return inputs
        #dA = gradient
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        zero_matrix = np.zeros_like(inputs[0])
        return np.maximum(inputs[0], zero_matrix)
    ...
    def backward(inputs, gradient):
        ...
        zero_matrix = np.zeros_like(x)

        return [np.where(x > 0, gradient, zero_matrix)]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        matrix_A = inputs[0]
        matrix_B = inputs[1]
        return [gradient*(matrix_A - matrix_B)/np.size(matrix_A), gradient*(matrix_B - matrix_A)/np.size(matrix_A)]
